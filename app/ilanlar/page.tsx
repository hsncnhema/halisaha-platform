    ...
    <div className="mx-auto max-w-2xl px-4 pb-16 pt-6">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-extrabold">📋 İlan Panosu</h1>
        </div>
        {kullanici ? (
          <button
            onClick={() => setFormAcik(!formAcik)}
            className={`rounded-xl px-4 py-2 text-sm font-bold transition ${
              formAcik ? 'bg-gray-100 text-gray-600 hover:bg-gray-200' : 'bg-green-600 text-white hover:bg-green-700'
            }`}
          >
      ...
      {formAcik && (
        <div className="mb-5 rounded-2xl border border-green-200 bg-white p-5">
          <h3 className="mb-4 text-base font-bold">Yeni İlan</h3>
          <form onSubmit={ilanAc} className="flex flex-col gap-3">
            <select
              value={form.kategori}
              onChange={(e) => setForm({ ...form, kategori: e.target.value })}
              className="w-full rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-sm focus:border-green-400 focus:outline-none"
            >
      ...
      {yukleniyor ? (
        ...
        <div className="rounded-2xl border border-gray-100 bg-white py-16 text-center">
          ...
          <p className="mt-1 text-sm text-gray-400">İlk ilanı sen aç!</p>
        </div>
      ) : (
        <div className="flex flex-col gap-3">
          {filtrelenmis.map((ilan) => (
            <div key={ilan.id} className="rounded-2xl border border-gray-100 bg-white p-4 transition hover:border-green-200">
              ...
              <h3 className="mb-1 text-sm font-bold">{ilan.baslik}</h3>
              <p className="mb-2 text-xs leading-relaxed text-gray-500">{ilan.aciklama}</p>
