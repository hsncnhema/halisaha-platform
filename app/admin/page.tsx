...
export default function AdminPage() {
  ...
  if (yukleniyor) {
    return (
      <div className="min-h-screen bg-green-950 mx-auto mt-24 max-w-4xl px-4 text-center">
        <p className="text-sm text-white/40">Yukleniyor...</p>
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-green-950 mx-auto max-w-4xl px-4 pb-16 pt-8">
      <div className="mb-6 flex items-start justify-between gap-3">
        <div>
          <h1 className="text-2xl font-extrabold text-white">Admin Onay Paneli</h1>
          <p className="mt-1 text-xs text-white/40">{adminAd}</p>
        </div>
        <Link href="/" className="text-sm font-semibold text-green-400 hover:underline">
          Ana sayfaya don
        </Link>
      </div>

      {hata && <p className="mb-4 rounded-lg border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm font-medium text-red-400">{hata}</p>}
      ...
      {bekleyenSahalar.length === 0 ? (
        ...
        <div className="space-y-3">
          {bekleyenSahalar.map((saha) => (
            <div key={saha.id} className="rounded-xl border border-white/10 bg-white/5 p-4">
              <div className="mb-3">
                <h2 className="text-base font-bold text-white">{saha.saha_adi}</h2>
                <p className="text-sm text-white/60">{saha.telefon}</p>
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  ...
                  className="rounded-lg bg-green-600 px-4 py-2 text-sm font-bold text-white transition hover:bg-green-700 disabled:cursor-not-allowed disabled:bg-white/10 disabled:text-white/30"
                >
                  Onayla
                </button>
                <button
                  ...
                  disabled={islemdeSahaId === saha.id}
                  className="rounded-lg border border-red-500/30 bg-red-500/10 px-4 py-2 text-sm font-bold text-red-400 transition hover:bg-red-500/20 disabled:cursor-not-allowed disabled:opacity-60"
                >
                  Reddet
                </button>
                <Link
                  href={`/saha/${saha.id}`}
                  target="_blank"
                  className="rounded-lg border border-white/10 bg-white/5 px-4 py-2 text-sm font-semibold text-white/60 transition hover:bg-white/10"
                >
                  Incele
                </Link>
              </div>
