...
export async function middleware(request: NextRequest) {
  ...
  const routeStartsWith = (basePath: string) => pathname === basePath || pathname.startsWith(`${basePath}/`);
  ...
  if (tip === 'saha') {
    ...
    const sahaYasak = [...sahaEngelliRotalar, ...authRotalari].some((route) => routeStartsWith(route));
